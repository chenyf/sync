...
$GLOBALS['LOG'] = array(
		...
		'strLogFile' => '/letv/log/cal.log',
		'arrSelfLogFiles' => array(
				'cal'       => '/letv/log/cal.sdf.log',
		),
);
...
$bookmarkBackend = new \Sabre\CalDAV\Backend\BookmarkPDO($GLOBALS["DB"]->getPDO());

# Directory structure
$nodes = array(
    new \Sabre\SimpleDAV\Principal\Collection($principalBackend),
    new \Sabre\SimpleDAV\BookmarkRootNode($principalBackend, $bookmarkBackend),
);
...
$server->addPlugin(new \Sabre\SimpleDAV\BookmarkPlugin());
$server->exec();
