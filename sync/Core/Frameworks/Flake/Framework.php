...
class Framework extends \Flake\Core\Framework {
	...
	protected static function initDbMysql() {
		...
		try {
            ...
            $GLOBALS["DB_SYNC"] = new \Flake\Core\Database\Mysql(
                PROJECT_DB_MYSQL_SYNC_HOST,
                PROJECT_DB_MYSQL_SYNC_DBNAME,
                PROJECT_DB_MYSQL_SYNC_USERNAME,
                PROJECT_DB_MYSQL_SYNC_PASSWORD
            );

			# We now setup the connexion to use UTF8
			$GLOBALS["DB"]->query("SET NAMES UTF8");

            $GLOBALS["DB_SYNC"]->query("SET NAMES UTF8");
		} catch(\Exception $e) {
